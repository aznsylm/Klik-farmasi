            ...
            <div class="card-body p-3">
                <div class="d-flex flex-wrap justify-content-between align-items-center">
                    <div class="d-flex gap-2 mb-2 mb-md-0">
                        <a href="{{ route('superadmin.users', ['role' => 'admin']) }}"
                            class="btn {{ $role == 'admin' ? 'btn-primary' : 'btn-outline-primary' }}">
                            Daftar Admin
                        </a>
                        <a href="{{ route('superadmin.users', ['role' => 'pasien']) }}"
                            class="btn {{ $role == 'pasien' ? 'btn-primary' : 'btn-outline-primary' }}">
                            Daftar Pasien
                        </a>
                    </div>
                    @if ($role == 'admin')
                        <a href="{{ route('superadmin.users.create', ['role' => 'admin']) }}" class="btn btn-primary">
                            Tambah Admin
                        </a>
                    @else
                        <a href="{{ route('superadmin.users.create', ['role' => 'pasien']) }}" class="btn btn-primary">
                            Tambah Pasien
                        </a>
                    @endif
                </div>
                            ...
                            @forelse($users as $i => $user)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        ...
                                        <a href="{{ route('superadmin.userEdit', $user->id) }}"
                                        ...
                                        <form action="{{ route('superadmin.deleteUser', $user->id) }}" method="POST"
                                            ...
                                            <button type="submit" class="btn btn-danger btn-sm"
                                                onclick="return confirm('Yakin hapus {{ $role == 'admin' ? 'admin' : 'pasien' }} ini?')"
                                                title="Hapus">
                                                Hapus
                                            </button>
                                        </form>
                            ...
                            @empty
                                <tr>
                                    <td colspan="8" class="text-center py-4">
                                        ...
                                        <span class="text-muted">Belum ada {{ $role == 'admin' ? 'admin' : 'pasien' }}
                                            terdaftar.</span>
                                    </td>
                                </tr>
                            @endforelse
